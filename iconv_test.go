...
func TestDecoding(t *testing.T) {
	...
	for i := range testData {
		decoder = getDecoder(testData[i].otherEncoding)
		if decoder == nil {
			decoded = testData[i].other
		} else {
			if decoded, err = decoder.String(testData[i].other); err != nil {
				t.Error(err)
			}
		}
		if strings.Compare(testData[i].utf8, decoded) != 0 {
			t.Errorf("decoded failed at %s -> %s", testData[i].utf8, decoded)
		}
	}
}

var testData = []struct { // from mahonia_test.go
